/*
* This file loads the Group Chat Manager and Connector Hyperties, both deployed from the Hyperty Domain Catalogue,
* Video Conference Application supported by reTHINK */
/**
...
// const arrayRuntimeTest = []
// const arrayHypertiesTest = []
// const promisesHypertiesTest = []
...
function ready (config) {
  ...
  script.onload = function () {
    ...
    //       console.log('[TEST]: Runtime has been successfully launched in ' + time + ' ms')
    //       // loadHyperty()
    //       const result = {total: time}
    //       arrayRuntimeTest.push(result)
    //       next(null, arrayRuntimeTest)
    //     })
    //   },
    //   (err, results) => {
    //     if (err) {
    ...
    //     }
    //     console.log('[TEST RESULT]:', arrayRuntimeTest)
    //   }
    // )
  }
...
function loadRuntime () {
  ...
  rethink.default.install({
    ...
    console.log('*** APP LOG ***: Runtime has been successfully launched in ' + time + ' seconds')
    loadHyperty()

    // HYPERTIES DEPLOYMENT TESTS
    // async.timesSeries(
    //   NUMBER_OF_EXECUTIONS,
    //   (n, next) => {
    //     const start = new Date().getTime()
        ...
        // Concorrently
        // promisesHypertiesTest.push(RUNTIME.requireHyperty(hypertyURI('localhost', 'GroupChatManager')))
        // promisesHypertiesTest.push(RUNTIME.requireHyperty(hypertyURI('localhost', 'Connector')))
        // Promise.all(promisesHypertiesTest).then(() => {
        //   const final = ((new Date().getTime()) - start)
        //   arrayHypertiesTest.push({concorrent: final})
        //   next(null, arrayHypertiesTest)
        // })

        // RUNTIME.requireHyperty(hypertyURI('hysmart.rethink.ptinovacao.pt', 'GroupChatManager')).then((groupChatManager) => {
        //   const chatTime = ((new Date().getTime()) - start)
        //   console.log('[TEST]: Hyperty ' + groupChatManager.name + ' Deployed in : ' + chatTime + ' ms')
        //   const restart = new Date().getTime()
        //   return RUNTIME.requireHyperty(hypertyURI('hysmart.rethink.ptinovacao.pt', 'Connector')).then((connector) => {  // NOTE 6
        //     const videoTime = ((new Date().getTime()) - restart)
        //     console.log('[TEST]: Hyperty ' + connector.name + ' Deployed in : ' + videoTime + ' ms')
        //     const result = {
        //       chat: chatTime,
        //       video: videoTime
        //     }
        //     arrayHypertiesTest.push(result)
        //
        //     next(null, arrayHypertiesTest)
        //   })
    ...
    //       return console.error(err)
    //     }
    //     console.log('[TEST RESULT]:', arrayHypertiesTest)
    //   }
    // )
  })
}
...
function hypertiesDeployed (GroupChatManager, Connector) {
  ...
  search.myIdentity().then(function (identity) {
    console.log('[IDENTITYS]: ', identity)
...
function hypertyReady (GroupChatManager, Connector, identity) {
  ...
  // console.log('[Loading Test] ', loadTime)
  ...
  //     if (readValue !== null) {
  //       value = value.concat(readValue)
  //     }
  //
  //     value.push({time: loadTime})
...
function prepareChat (chatController) {
  chatController.onMessage(function (message) {
    ...
    //messagesDelivery(message.value.content, time)
  })
...
/*
  Messages Delivery Test
*/
// function messagesSend (chatController) {
//   // Send message
//   let sendMessage = $('#send-message')
//   sendMessage.on('click', function (event) {
//     event.preventDefault()
...
//           chatController.send(start).then(function (result) {
//             next(null, arrayMessagesTest)
//           }).catch(function (reason) {
//             console.error('Error while sending a message: ', reason)
//           })
//         }, 500)
...
/*
  Messages Delivery Test
*/
// function messagesDelivery (start, time) {
...
function processParticipants (event) {
  ...
  if (user.username !== $('.email').text().split('Email: ')[1]) {
    ...
    $('#chat-board').append('<p style=color:red;> *** ' + user.username + ' entered the chat! ***</p>')
  }
}
